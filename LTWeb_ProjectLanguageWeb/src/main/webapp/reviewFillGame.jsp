...
<link rel="stylesheet" href="./css/gamefill.css">
		...
		<div class="row">
			...
			<div style="background-color: #f2f2f266" class="col-6 col_2_content">
				<form action="<c:url value="reviewgame"/>" method="get">
					...
					<div id="body-game">
						<div id="gamefill" class="title">Điền từ</div>
						<p style="margin: 20px 0" class="vocabulary">
							${currentWord.mean} <span>(n)</span>
						</p>
						<div class="box-inputs">
							<span class="wrapper-input">
								<c:set var = "index" scope = "session" value = "${0}"/>
								<c:forEach items="${arrWord}" var="word" varStatus="loop">
									<c:choose>
										<c:when test="${word != ' '}">
											<c:set var = "index" scope = "session" value = "${index+1}"/>
											<input id="input-${index}" maxlength="1" data="${index}"  autocomplete="off" type="text" class="fillword input">
										</c:when>
										<c:when test="${word == ' '}"><span id="space" class ="input" type="space"></span></c:when>	
									</c:choose>
								</c:forEach>	
							</span>				
						</div>
